...
public:
    ...
    void setMatrix(const ColMatrix &mat);

    float chi2() const;
    uint64_t nAtoms() const;

    void sync(const GibbsSampler &other);
...
private:
    ...
    void acceptExchange(Atom *atom1, Atom *atom2, float delta, unsigned r1,
        unsigned c1, unsigned r2, unsigned c2);
    bool conditionallyUpdateAtom(Atom *atom, float delta);
    ...
    bool canUseGibbs(unsigned pattNdx) const;
    bool canUseGibbs(unsigned pattNdx1, unsigned pattNdx2) const;

    AlphaParameters alphaParameters(unsigned mainNdx, unsigned patNdx);
    AlphaParameters alphaParameters(unsigned mainNdx1, unsigned patNdx1,
        unsigned mainNdx2, unsigned pattNdx2);

    float computeDeltaLL(unsigned mainNdx, unsigned patNdx, float delta);
    float computeDeltaLL(unsigned mainNdx1, unsigned patNdx1, float delta1,
        unsigned mainNdx2, unsigned pattNdx2, float delta2);
};

template <class DataType>
GibbsSampler::GibbsSampler(const DataType &data, bool transposeData,
...
template <class DataType>
void GibbsSampler::setUncertainty(const DataType &unc, bool transpose,
bool partitionRows, const std::vector<unsigned> &indices)
{
